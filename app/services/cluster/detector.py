...
C006 执行流程：
1. 检查簇组总开关是否开启
2. 由 ClusterManager.match_cluster 在内存里基于 signature 索引匹配
3. 匹配成功：
...
class ClusterDetectionResult:
    ...
    def __init__(
        ...
        should_continue_detection: bool = True,  # 是否继续执行 C001-C005
...
def run_cluster_detection(
    ...
    Args:
        db: 数据库 session
        features: TxFeatures（已在外层提取，不再重复）
        manager: ClusterManager（可选，由外层传入以避免重复创建）
    ...
    if manager is None:
        try:
            from app.services.trade_processor import get_global_manager
            manager = get_global_manager()
        except Exception:
            # 兜底：fallback 到 create_manager(db)
            manager = create_manager(db)

    # 尝试匹配已有簇组（features 已在外层提取，不再重复）
    match_result = manager.match_cluster(features)
