...
class TradeBackfill:
                ...
                while self.running and total_saved < max_total:
                    ...
                    for tx in txs:
                        try:
                            ...
                            # ========= 方案 B：buy/sell 闸门 ----------
                            # 在 _fetch_all_transactions_before 里。逻辑与 stream 一致。
                            ...
                            if not (is_buy or is_sell):
                                self._log_dirty_sample(sig, tx.get("transaction", {}), meta)
                                continue
    ...
    def _log_dirty_sample(self, sig: str, transaction: Any, meta: Any) -> None:
        """
        脏样本捕获器：与 trade_stream 同逻辑。
        用于 在 BUY/SELL 闸门丢掉非买卖交易后，打印 main/inner 里非 dict 的元素。
        """
        try:
            try:
    ...
    def _resolve_trader_owner(
        ...
        """与 trade_stream 同逻辑（补丁 v3）：signer 优先 → post 优先 → pre 优先。

        补丁 v3（2026-07-09）：signer 优先分支
        - 修复 pump.fun SELL 全卖光被错判为 BUY 的 bug。
        - 当 signer 本身在 pre/post 上有 target_mint 余额时，锁定 trader=signer，
          避免被池子（accountIndex 在前、post 余额增加）抢先匹配。
        """
        try:
            # ===== 补丁 v3：signer 优先 =====
            ...
            logger.debug(f"[回填] 解析 trader owner 失败: {e}")
    ...
    def _extract_instruction_details(self, message: Dict[str, Any], meta: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for idx, ix in enumerate(instructions):
                # 补丁 v2：isinstance 守卫，遇到非 dict 元素（比如 RPC 返回里是字符串）跳过但不崩
                if not isinstance(ix, dict):
                    continue
                program_id = ix.get("programId", "")
    ...
    def _extract_trade_info(self, tx: Dict[str, Any], gate_decision: str = "UNKNOWN") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # ===== SOL 流向计算（净值）补丁 v2：双抽 fee + jito_tip =====
            ...
            if (owner_index is not None and pre_sol_balances and post_sol_balances
                ...
                # 同 stream 逻辑（补丁 v2 修正）：加回 fee + jito_tip
                # diff(post - pre) 已是扣完手续费后的净值，要还原真实 SOL 流向需加回 fee + tip
            ...
            try:
                if abs(delta) > 1e15:
                    logger.warning(f"[回填] amount 超出安全范围 sig={sig[:8]} delta={delta:.4e}")
                    amount = 0.0
                else:
            ...
            # ===== 交易类型判定（资金流向优先）补丁 v2：from_addr 改 trader_owner =====
            tx_type = "TRANSFER"
            ...
            # ===== wSOL 余额变化修正 sol_spent（DEX 交易资金主力）补丁 v2：用 trader_owner =====
            WSOL_MINT = "So11111111111111111111111111111111111111112"
            wsol_pre = wsol_post = 0.0
            for b in post_token_list:
                if b.get("mint") == WSOL_MINT and b.get("owner") == trader_owner:
            ...
            if tx_type == "SELL" and wsol_delta > 0:
                ...
                sol_spent = wsol_delta  # wsol_delta 为负
            ...
            result_data = {
                ...
                # 2026-07-09 OOM 修复：raw_data 不再写进 Redis。完整 tx 已被 _extract_trade_info
                # 消费,后续没人读 raw_data 字段。关闭后每条 detail ~20KB → ~1KB，Redis 内存 -95%。
                "source": "rpc_fill",
            }
