"""
SIG 顺序验证测试
对比 Solana RPC 真实顺序 vs Redis 存储顺序
验证 WSS 接收数据是否正确写入 Redis
"""
import asyncio
import json
import logging
...
async def get_redis_signatures(mint: str) -> list[dict]:
    """从 Redis 获取签名列表"""
...
def compare_order(rpc_sigs: list[dict], redis_sigs: list[dict]):
    """对比 RPC 和 Redis 的顺序"""
    results = []
    results.append("=" * 80)
    ...
    results.append(f"  Redis 记录数: {len(redis_sigs)}")
    results.append(f"  差异（成功交易 - Redis）: {len(rpc_success) - len(redis_sigs)}")
    results.append("")

    # 2. 完整性检查（只检查成功的交易）
    results.append("【2. 完整性检查 - 成功的 RPC 签名是否全部入库】")
    rpc_sig_set = {s["signature"] for s in rpc_success}
    redis_sig_set = {s["sig"] for s in redis_sigs}

    missing_in_redis = rpc_sig_set - redis_sig_set
    extra_in_redis = redis_sig_set - rpc_sig_set

    if missing_in_redis:
        results.append(f"  缺失（RPC 成功但 Redis 无）: {len(missing_in_redis)} 条")
        for sig in list(missing_in_redis)[:5]:
            results.append(f"    - {sig[:16]}...")
        if len(missing_in_redis) > 5:
            results.append(f"    ... 还有 {len(missing_in_redis) - 5} 条")
    else:
        results.append("  通过: 所有成功的 RPC 签名均已入库")

    if extra_in_redis:
        results.append(f"  多余（Redis 有但 RPC 无）: {len(extra_in_redis)} 条")
        for sig in list(extra_in_redis)[:5]:
            results.append(f"    - {sig[:16]}...")
    else:
        results.append("  通过: Redis 无多余签名")
    ...
    redis_list = [s["sig"] for s in redis_sigs]

    common_sigs = [s for s in rpc_list if s in redis_sig_set]
    redis_common = [s for s in redis_list if s in rpc_sig_set]

    order_match = common_sigs == redis_common
    results.append(f"  交集签名数: {len(common_sigs)}")
    results.append(f"  顺序是否一致: {'是' if order_match else '否'}")

    if not order_match:
        for i, (r, d) in enumerate(zip(common_sigs, redis_common)):
            if r != d:
                results.append(f"  第一个差异位置: 索引 {i}")
                results.append(f"    RPC 顺序: {r[:16]}...")
                results.append(f"    Redis 顺序: {d[:16]}...")
                break
    results.append("")

    # 4. Slot 对应关系
    results.append("【4. Slot 对应关系（前 10 条）】")
    results.append(f"  {'#':>3}  {'RPC Slot':>15}  {'Redis Slot':>15}  {'一致':>6}  {'RPC Sig':>16}  {'Redis Sig':>16}")
    ...
    for i in range(min(10, len(rpc_success))):
        ...
        if i < len(redis_sigs):
            redis_sig = redis_sigs[i]["sig"]
            redis_slot = redis_sigs[i].get("slot", 0) or 0
            match = "OK" if rpc_slot == redis_slot else "MISMATCH"
            results.append(f"  {i+1:>3}  {rpc_slot:>15}  {redis_slot:>15}  {match:>6}  {rpc_sig[:16]:>16}  {redis_sig[:16]:>16}")
        else:
            results.append(f"  {i+1:>3}  {rpc_slot:>15}  {'N/A':>15}  {'MISS':>6}  {rpc_sig[:16]:>16}  {'N/A':>16}")
    results.append("")

    # 5. WSS 实时写入验证
    results.append("【5. WSS 实时写入验证】")
    results.append("  按 source 分类:")
    ws_sigs_list = [s for s in redis_sigs if s.get("source") == "ws"]
    rpc_sigs_list = [s for s in redis_sigs if s.get("source") == "rpc_fill"]
    results.append(f"  回填占位记录数 (rpc_fill): {len(rpc_sigs_list)}")
    results.append(f"  WSS 入库记录数 (ws): {len(ws_sigs_list)}")

    if ws_sigs_list:
        results.append(f"  最新 WSS 记录: sig={ws_sigs_list[0]['sig'][:16]}..., slot={ws_sigs_list[0]['slot']}")
        results.append(f"  最旧 WSS 记录: sig={ws_sigs_list[-1]['sig'][:16]}..., slot={ws_sigs_list[-1]['slot']}")

        ws_sig_set = {s["sig"] for s in ws_sigs_list}
    ...
    results.append("=" * 80)
    results.append("【总结】")
    total_ok = len(missing_in_redis) == 0 and order_match
    results.append(f"  完整性: {'通过' if len(missing_in_redis) == 0 else '失败'}")
...
async def main():
    ...
    # 3. 从 Redis 获取签名
    logger.info("===== 步骤 3: 从 Redis 获取签名 =====")
    redis_sigs = await get_redis_signatures(TEST_MINT)
    logger.info(f"Redis 查询完成: {len(redis_sigs)} 条")

    # 4. 对比
    logger.info("===== 步骤 4: 对比分析 =====")
    report = compare_order(rpc_sigs, redis_sigs)
    print(report)
